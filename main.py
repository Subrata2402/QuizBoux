...
class MainClass(commands.Cog, Websocket):
    ...
    async def help(self, ctx):
        ...
        embed.add_field(name = f"{ctx.prefix}login [username] [password]", value = "Login to Display for start websocket. Before login please read the note carefully.\n**__Note :__** For Display Trivia, please don't use username and password of main account for the chances of account ban. If account will get ban then we are not responsible for this.", inline = False)
        ...
        embed.add_field(name = f"{ctx.prefix}start [mimir/display]", value = "Start Websocket of Mimir/Display. Before start the display websocket please read the note carefully.\n**__Note :__** Turn on the websocket 30 seconds before the question arrives or when the question comes up. If started before long time the question coming, close it and start again.", inline = False)
